...
| **— Strands Agentic AI —**               |                                                                                                                                                                                                                                                                                                                                    |
| `STRANDS_AGENT_RUNTIME.md`               | Strands SDK agent runtime (custom @tool, multi-agent supervisor/worker, Bedrock/Anthropic/OpenAI model configs), Lambda + ECS agent hosts, DynamoDB sessions, S3 artifacts                                                                                                                                                         |
| `STRANDS_AGENTCORE_DEPLOY.md`            | AgentCore managed deployment (BedrockAgentCoreApp wrapper), Gateway MCP endpoint (OAuth2 + Cognito), Memory primitives (STM + LTM), agentcore CLI workflow                                                                                                                                                                         |
| `STRANDS_AGENT_FRONTEND.md`              | Streaming chat UI (WebSocket API GW v2), React chat hook + component, REST fallback, DynamoDB connection management, session CRUD endpoints                                                                                                                                                                                        |
| `STRANDS_AGENT_EVAL.md`                  | Agent evaluation harness (golden datasets, LLM-as-judge scoring), Step Functions eval orchestration, prompt regression detection, CloudWatch score dashboard, CICD quality gate                                                                                                                                                     |
...
| "Strands SDK", "custom AI agent", "tool-use agent"                       | Strands agent runtime + Lambda/ECS host + DynamoDB sessions          |
| "multi-agent", "supervisor agent", "agent orchestration"                 | Strands multi-agent supervisor + worker pattern                      |
| "AgentCore", "MCP tools", "agent memory", "STM", "LTM"                  | AgentCore deployment + Gateway MCP + Memory primitives               |
| "agent chat UI", "conversational interface", "AI assistant UI"           | WebSocket API GW v2 + React streaming chat + session management      |
| "agent eval", "golden dataset", "prompt regression", "eval harness"      | Step Functions eval pipeline + LLM judge + CICD quality gate         |
...
- **Strands @tool Pattern** — All agent tools use `@tool` decorator with typed docstrings — Strands auto-generates the tool schema
- **Agent Eval Quality Gate** — No agent reaches production without passing golden dataset eval (LLM-as-judge + assertion checks)
- **AgentCore for Managed Hosting** — Use AgentCore (not raw Lambda/ECS) when SOW mentions managed agent deployment, Gateway MCP, or agent memory
- **WebSocket for Agent Streaming** — Agent chat UIs use API Gateway v2 WebSocket for real-time token streaming, REST as fallback
